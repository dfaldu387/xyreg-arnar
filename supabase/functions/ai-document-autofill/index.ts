...
2. Carefully read ALL the reference document content. Look for any information that could be relevant to the requested section — even if it is not explicitly labeled with the same section name. For example, content about "project boundaries" is relevant to a "Scope" section.
3. Always generate content for the section. Extract and reorganize relevant information from the reference documents into the appropriate section structure.
4. If the reference documents contain limited information for the section, generate content from what IS available and add a note at the end: <p><em><strong>Note:</strong> The reference documents contain limited information for this section. The following topics may need additional detail: [list topics].</em></p>
5. Structure the content professionally for a medical device QMS document. Reference standards (ISO 13485, FDA 21 CFR Part 820, EU MDR) where the reference documents mention or relate to them.
6. Do NOT use generic placeholder text like "[Insert Company Name]" or "[Insert Product Name]". If specific details are in the reference documents, use them. If not, omit them.
...
--- REFERENCE DOCUMENTS END ---`;
    ...
    const userPrompt = prompt || `Generate content for the "${sectionTitle}" section of the "${documentName || 'Untitled'}" document (type: ${documentType || 'QMS'}). Extract all relevant information from the reference documents for this section.`;
