/**
 * AI Hardware Requirements Generator Edge Function
 *
 * Uses Google Vertex AI (Gemini) to generate hardware requirements from system requirements.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("[ai-hardware-requirements-generator] Starting generation");
    ...
    const existingItemsSection = existingItems?.length > 0
      ? `\n\nEXISTING HARDWARE REQUIREMENTS (DO NOT suggest again):\n${existingItems.map((i: string) => `- "${i}"`).join("\n")}\n\nGenerate ONLY NEW requirements.`
      : "";

    const systemPrompt = `You are a Hardware Lead for medical device development. Translate system requirements into specific, actionable hardware requirements.${existingItemsSection}
...
CATEGORIES: ${selectedCategories?.length > 0 ? selectedCategories.join(", ") : "Electrical, Mechanical, Environmental, Safety, Performance, Materials"}

Generate 5-8 hardware requirements. Return ONLY a JSON array:
[
  {
    ...
    "acceptance_criteria": "Specific hardware testing criteria",
    "confidence": 0.85
  }
    ...
    const requestBody = JSON.stringify({
      contents: [{ role: "user", parts: [{ text: "Generate hardware requirements that implement these system requirements following IEC 60601-1 and ISO 10993 standards." }] }],
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (!vertexResponse.ok) {
        ...
        console.error("[ai-hardware-requirements-generator] Vertex error:", vertexResponse.status, errorText);
      ...
      console.log(`[ai-hardware-requirements-generator] Attempt ${attempt + 1}: parts=${parts.length}, responseLen=${aiResponse.length}`);
      ...
      try {
        ...
        console.error(`[ai-hardware-requirements-generator] Parse attempt ${attempt + 1} failed:`, e);
    ...
    if (companyId && vertexData) {
      ...
      if (detailedUsage) {
        EdgeRuntime.waitUntil(Promise.all([
          logAiTokenUsage({ companyId, userId, source: "ai_hardware_requirements", model: MODEL, usage: detailedUsage, metadata: { productName: productData.product_name ?? null } }),
    ...
    console.log("[ai-hardware-requirements-generator] Generated", suggestions.length, "suggestions");
