-- Create market extensions table for platform + market model
...
CREATE TABLE public.rnpv_calculations (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  scenario_id UUID NOT NULL REFERENCES public.rnpv_scenarios(id) ON DELETE CASCADE,
  product_id UUID NOT NULL,
  company_id UUID NOT NULL,
...
CREATE INDEX idx_market_extensions_product ON public.market_extensions(product_id);
CREATE INDEX idx_market_extensions_company ON public.market_extensions(company_id);
...
CREATE INDEX idx_rnpv_calculations_product ON public.rnpv_calculations(product_id);
CREATE INDEX idx_expert_assessments_product ON public.expert_loa_assessments(product_id);
...
-- RLS Policies for market_extensions
CREATE POLICY "Users can view market extensions for their companies" 
ON public.market_extensions FOR SELECT 
USING (company_id IN (
...
ON public.market_extensions FOR INSERT 
...
ON public.market_extensions FOR UPDATE 
USING (company_id IN (
  SELECT user_company_access.company_id
  FROM user_company_access
  WHERE user_company_access.user_id = auth.uid() 
...
ON public.market_extensions FOR DELETE 
USING (company_id IN (
  SELECT user_company_access.company_id
  FROM user_company_access
  WHERE user_company_access.user_id = auth.uid() 
...
CREATE POLICY "Users can delete rNPV calculations for their companies" 
ON public.rnpv_calculations FOR DELETE 
USING (company_id IN (
  SELECT user_company_access.company_id
  FROM user_company_access
  WHERE user_company_access.user_id = auth.uid() 
...
CREATE POLICY "Users can delete expert assessments for their companies" 
ON public.expert_loa_assessments FOR DELETE 
USING (company_id IN (
  SELECT user_company_access.company_id
  FROM user_company_access
  WHERE user_company_access.user_id = auth.uid() 
...
CREATE TRIGGER update_market_extensions_updated_at
  BEFORE UPDATE ON public.market_extensions
