...
-- Handle duplicates more thoroughly

-- 1. Drop any existing partial unique indexes
DROP INDEX IF EXISTS unique_company_gap_analysis_items;

-- 2. Delete ALL existing gap analysis items where product_id IS NULL
-- This ensures complete cleanup regardless of framework
DELETE FROM gap_analysis_items 
WHERE product_id IS NULL;

-- 3. Regenerate gap analysis items from the enabled template
...
CREATE UNIQUE INDEX unique_company_gap_analysis_items 
