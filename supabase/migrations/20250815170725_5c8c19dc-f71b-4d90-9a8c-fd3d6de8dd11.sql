...
SELECT 
  ...
  -- Simple encryption using XOR and base64 (cast setting_value to text first)
  encode(
    convert_to(
      array_to_string(
        ARRAY(
          SELECT chr(
            ascii(substring(ts.setting_value::text, generate_series(1, length(ts.setting_value::text)), 1)) # 
            ascii(substring('medtech-api-key-2024', ((generate_series(1, length(ts.setting_value::text)) - 1) % length('medtech-api-key-2024')) + 1, 1))
          )
        ), 
        ''
      ),
...
WHERE ts.setting_key IN ('gemini_api_key', 'openai_api_key', 'anthropic_api_key')
  ...
  AND ts.setting_value::text != ''
  AND ts.setting_value::text != 'null'
