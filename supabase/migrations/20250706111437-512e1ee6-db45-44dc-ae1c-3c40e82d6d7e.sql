...
-- First, let's check what foreign key constraints currently exist on lifecycle_phases
SELECT 
  tc.constraint_name, 
  tc.table_name, 
  kcu.column_name, 
...
-- Drop any existing foreign key constraints on phase_id that might be referencing the wrong table
...
ALTER TABLE lifecycle_phases DROP CONSTRAINT IF EXISTS lifecycle_phases_phase_id_key;

-- Create the correct foreign key constraint that references company_phases
