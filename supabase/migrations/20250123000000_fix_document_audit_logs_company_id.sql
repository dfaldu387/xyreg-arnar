-- Fix the log_reviewer_assignment_activity function to properly handle company_id
-- The issue was on line 34 where document_id_val was being used instead of company_id from document
...
BEGIN
  ...
  IF workflow_data.record_type = 'document' THEN
    ...
    -- Use document company_id if available (FIXED: was using document_id_val instead of company_id_val)
    company_id_val := COALESCE(company_id_val, reviewer_group_data.company_id, NEW.company_id, OLD.company_id);
  ...
  IF TG_OP = 'INSERT' THEN
    -- Log to document_audit_logs only if this is a document workflow AND we have a valid company_id
...
-- Also fix the log_review_decision_activity function to ensure it handles company_id properly
...
COMMENT ON FUNCTION log_reviewer_assignment_activity() IS 'Fixed: Properly handles company_id from document table instead of using document_id_val as company_id';
COMMENT ON FUNCTION log_review_decision_activity() IS 'Fixed: Ensures company_id is properly validated before inserting into document_audit_logs';
