-- Mission Control Database Schema - Handle Existing Tables
-- Creates remaining company-specific tables for Mission Control dashboard sections
...
CREATE TABLE IF NOT EXISTS mission_requires_attention (
...
-- Action Items table
CREATE TABLE IF NOT EXISTS mission_action_items (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  title TEXT NOT NULL,
  description TEXT,
...
CREATE TABLE IF NOT EXISTS mission_activity_stream (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS mission_executive_communications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  title TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS mission_recent_messages (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_mission_requires_attention_company_id ON mission_requires_attention(company_id);
CREATE INDEX IF NOT EXISTS idx_mission_requires_attention_status ON mission_requires_attention(status);
CREATE INDEX IF NOT EXISTS idx_mission_action_items_company_id ON mission_action_items(company_id);
CREATE INDEX IF NOT EXISTS idx_mission_action_items_assigned_to ON mission_action_items(assigned_to);
CREATE INDEX IF NOT EXISTS idx_mission_action_items_status ON mission_action_items(status);
CREATE INDEX IF NOT EXISTS idx_mission_action_items_due_date ON mission_action_items(due_date);
CREATE INDEX IF NOT EXISTS idx_mission_activity_stream_company_id ON mission_activity_stream(company_id);
CREATE INDEX IF NOT EXISTS idx_mission_activity_stream_created_at ON mission_activity_stream(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_mission_executive_communications_company_id ON mission_executive_communications(company_id);
CREATE INDEX IF NOT EXISTS idx_mission_executive_communications_published ON mission_executive_communications(is_published, published_at DESC);
CREATE INDEX IF NOT EXISTS idx_mission_recent_messages_company_id ON mission_recent_messages(company_id);
CREATE INDEX IF NOT EXISTS idx_mission_recent_messages_thread_id ON mission_recent_messages(thread_id);
CREATE INDEX IF NOT EXISTS idx_mission_recent_messages_created_at ON mission_recent_messages(created_at DESC);

-- Create triggers for updated_at timestamps (avoid duplicates)
...
DROP TRIGGER IF EXISTS update_mission_requires_attention_updated_at ON mission_requires_attention;
DROP TRIGGER IF EXISTS update_mission_action_items_updated_at ON mission_action_items;
DROP TRIGGER IF EXISTS update_mission_executive_communications_updated_at ON mission_executive_communications;
DROP TRIGGER IF EXISTS update_mission_recent_messages_updated_at ON mission_recent_messages;
...
-- Portfolio Health policies (may already exist)
DO $$ BEGIN
