-- Product Portfolio Structure and Product Variations
-- Create tables for company-level variation dimensions and options, and product-level variants and values
...
-- Dimensions
CREATE POLICY IF NOT EXISTS product_variation_dimensions_select ON public.product_variation_dimensions
  FOR SELECT USING (
    company_id IN (
...
CREATE POLICY IF NOT EXISTS product_variation_dimensions_insert ON public.product_variation_dimensions
...
CREATE POLICY IF NOT EXISTS product_variation_dimensions_update ON public.product_variation_dimensions
  FOR UPDATE USING (
    company_id IN (
      SELECT uca.company_id FROM public.user_company_access uca 
      WHERE uca.user_id = auth.uid() AND uca.access_level IN ('admin','editor')
...
CREATE POLICY IF NOT EXISTS product_variation_dimensions_delete ON public.product_variation_dimensions
  FOR DELETE USING (
    company_id IN (
      SELECT uca.company_id FROM public.user_company_access uca 
      WHERE uca.user_id = auth.uid() AND uca.access_level IN ('admin','editor')
...
CREATE POLICY IF NOT EXISTS product_variation_options_select ON public.product_variation_options
  FOR SELECT USING (
    company_id IN (
      SELECT uca.company_id FROM public.user_company_access uca WHERE uca.user_id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS product_variation_options_insert ON public.product_variation_options
  FOR INSERT WITH CHECK (
    company_id IN (
      SELECT uca.company_id FROM public.user_company_access uca 
      WHERE uca.user_id = auth.uid() AND uca.access_level IN ('admin','editor')
...
CREATE POLICY IF NOT EXISTS product_variation_options_update ON public.product_variation_options
  FOR UPDATE USING (
    company_id IN (
      SELECT uca.company_id FROM public.user_company_access uca 
      WHERE uca.user_id = auth.uid() AND uca.access_level IN ('admin','editor')
...
CREATE POLICY IF NOT EXISTS product_variation_options_delete ON public.product_variation_options
  FOR DELETE USING (
    company_id IN (
      SELECT uca.company_id FROM public.user_company_access uca 
      WHERE uca.user_id = auth.uid() AND uca.access_level IN ('admin','editor')
...
CREATE POLICY IF NOT EXISTS product_variants_select ON public.product_variants
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS product_variants_insert ON public.product_variants
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.products p 
      JOIN public.user_company_access uca ON uca.company_id = p.company_id
...
CREATE POLICY IF NOT EXISTS product_variants_update ON public.product_variants
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.products p 
      JOIN public.user_company_access uca ON uca.company_id = p.company_id
...
CREATE POLICY IF NOT EXISTS product_variants_delete ON public.product_variants
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.products p 
      JOIN public.user_company_access uca ON uca.company_id = p.company_id
...
CREATE POLICY IF NOT EXISTS product_variant_values_select ON public.product_variant_values
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS product_variant_values_insert ON public.product_variant_values
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.product_variants v
      JOIN public.products p ON p.id = v.product_id
...
CREATE POLICY IF NOT EXISTS product_variant_values_update ON public.product_variant_values
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.product_variants v
      JOIN public.products p ON p.id = v.product_id
...
CREATE POLICY IF NOT EXISTS product_variant_values_delete ON public.product_variant_values
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.product_variants v
      JOIN public.products p ON p.id = v.product_id
