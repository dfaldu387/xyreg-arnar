...
-- CORRECTED: ARCHIVE ALL PHASE-ASSIGNED AND UNASSIGNED COMPANY "TEMPLATES" (FIXED)
-- This script will:
-- 1. Archive current data (only if data exists)
-- 2. Clean up foreign key references first
-- 3. Delete templates in the correct order

BEGIN;

-- Step 1: Archive current phase_assigned_documents (company_template scope) - only if data exists
...
-- Step 2: Archive current company_document_templates - only if data exists
INSERT INTO archived_pms_data (table_name, archived_data, migration_phase)
SELECT 
...
-- Step 3: Archive current documents with scope=company_template - only if data exists
INSERT INTO archived_pms_data (table_name, archived_data, migration_phase)
SELECT 
...
-- Step 4: Archive documents that reference phase_assigned_documents as template_source_id - only if data exists
INSERT INTO archived_pms_data (table_name, archived_data, migration_phase)
SELECT 
