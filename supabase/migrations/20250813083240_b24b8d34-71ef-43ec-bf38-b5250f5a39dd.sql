-- Pricing Strategy Module - Phase 1 (Schema, RLS, functions, triggers)
-- Safe guards for types
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS public.pricing_rules (
  ...
  -- product_variant_id uuid REFERENCES public.product_variants(id) ON DELETE CASCADE, -- optional future use
  model_name text,
  ...
  CONSTRAINT pricing_rules_scope_xor CHECK (
    ((product_id IS NOT NULL)::int + (model_name IS NOT NULL)::int) = 1
  ),
...
-- Unique constraints per scope/market
...
CREATE POLICY IF NOT EXISTS "pricing_rules_select"
...
CREATE POLICY IF NOT EXISTS "pricing_rules_insert"
  ON public.pricing_rules
  FOR INSERT
  WITH CHECK (
    company_id IN (
...
CREATE POLICY IF NOT EXISTS "pricing_rules_update"
  ON public.pricing_rules
  FOR UPDATE
  USING (
    company_id IN (
...
CREATE POLICY IF NOT EXISTS "pricing_rules_delete"
  ON public.pricing_rules
  FOR DELETE
  USING (
    company_id IN (
...
CREATE POLICY IF NOT EXISTS "pricing_effective_select"
...
-- Utility trigger to update updated_at on pricing_rules
...
-- Function: compute effective price for a single product
...
BEGIN
  ...
  SELECT id, company_id, parent_product_id, model_reference
  INTO v_product
  ...
  LOOP
    ...
    IF FOUND THEN
      IF v_rule.rule_type IN ('BASE','ABSOLUTE') THEN
        ...
        v_path := v_path || ('product:' || v_product.id || '=' || v_rule.rule_type);
      ELSIF v_rule.rule_type = 'RELATIVE' THEN
        -- Apply relative only if we already have an anchor price
        IF v_price IS NULL THEN
          -- Try model-level anchor at this node
          IF v_product.model_reference IS NOT NULL THEN
            SELECT r.* INTO v_model_rule
            FROM public.pricing_rules r
            WHERE r.company_id = v_company_id
              AND r.market_code = v_market
              AND r.model_name ILIKE v_product.model_reference
            LIMIT 1;
            IF FOUND AND v_model_rule.rule_type IN ('BASE','ABSOLUTE') THEN
              ...
              v_path := v_path || ('model:' || v_product.model_reference || '=' || v_model_rule.rule_type);
            END IF;
          END IF;
        END IF;
        ...
        IF v_price IS NOT NULL THEN
          ...
          END IF;
          v_currency := v_rule.currency_code; -- Keep rule's currency context
          v_source_rule := v_rule.id;
          v_path := v_path || ('product:' || v_product.id || '=RELATIVE');
        END IF;
      END IF;
    END IF;
  END LOOP;
  ...
  IF v_price IS NOT NULL THEN
    ...
    DO UPDATE SET
      ...
      computed_at = now();
  ELSE
    -- No price resolvable: remove any stale entry
...
-- Function: recompute prices for a company (optionally for a market; null = all markets in rules or 'US' fallback)
...
DECLARE
  ...
  v_product_id uuid;
BEGIN
  -- Determine markets to recompute
...
-- Trigger to recompute on pricing_rules change (simple: recompute company + market)
...
-- Trigger to recompute when product inheritance-related fields change
