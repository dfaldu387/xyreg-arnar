...
interface SystemRequirementsImportDialogProps {
  open: boolean;
...
export function SystemRequirementsImportDialog({ open, onOpenChange, productId, companyId }: SystemRequirementsImportDialogProps) {
  ...
  const downloadTemplate = (e: React.MouseEvent) => {
    ...
    const templateData = [
      ...
      ['System shall maintain measurement accuracy ±1%', 'System Use', 'UN-C-01', 'HAZ-SYS-001', 'Not Started', ''],
      ['System shall operate within 15-35°C ambient', 'Environmental', 'UN-DR-01', '', 'Not Started', ''],
      ['System shall provide visual and audible alarms', 'Safety', 'UN-DR-02', 'HAZ-SYS-002', 'Not Started', 'DHS-ALM-100'],
      ['System shall withstand 1.2m drop test', 'Mechanical', 'UN-C-03', '', 'Not Started', ''],
      ['System shall complete self-test within 30s', 'Software', 'UN-PD-01', '', 'Not Started', ''],
    ];
    ...
    const instructionsData = [
      ...
      ['Description', 'Yes', 'The system requirement statement', 'Any text'],
      ['Category', 'No', 'Requirement category for grouping', validCategories],
      ['Traces To', 'No', 'User Need IDs this derives from (comma-separated)', 'e.g. UN-C-01, UN-DR-02'],
      ['Linked Risks', 'No', 'Hazard IDs mitigated by this requirement (comma-separated)', 'e.g. HAZ-SYS-001'],
      ['Verification Status', 'No', 'Current verification state (defaults to Not Started)', 'Not Started, In Progress, Passed, Failed'],
      ['Linked Component Part Number', 'No', 'Part number or name of linked device component', 'e.g. DHS-ALM-100'],
    ];
    ...
    XLSX.writeFile(wb, 'system-requirements-import-template.xlsx');
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ['requirement-specifications', productId, 'system'] });
      toast.success(`Imported ${succeeded} system requirements${linked > 0 ? `, ${linked} linked to components` : ''}`);
      handleClose(false);
    } catch (err: any) {
      console.error('SYSR import failed:', err);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh]">
        ...
        {step === 'upload' && (
          <div
            ...
            onDrop={handleDrop}
            onClick={() => document.getElementById('sysr-import-file')?.click()}
          >
            ...
            <input id="sysr-import-file" type="file" accept=".csv,.xlsx,.xls" className="hidden" onChange={handleFileInput} />
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {unmatchedCount > 0 && (
              <div className="flex items-start gap-2 text-sm text-orange-700 bg-orange-50 border border-orange-200 rounded-md p-3">
                ...
                <span>{unmatchedCount} rows reference component part numbers not found in the device architecture. These will be imported without a component link.</span>
              </div>
            )}
            <p className="text-sm text-muted-foreground">
              Each requirement will receive an auto-generated SYSR-XXX ID. Requirements are inserted sequentially to ensure unique IDs.
            </p>
          </div>
        )}
