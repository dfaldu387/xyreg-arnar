...
interface SoftwareRequirementsImportDialogProps {
  open: boolean;
...
export function SoftwareRequirementsImportDialog({ open, onOpenChange, productId, companyId }: SoftwareRequirementsImportDialogProps) {
  ...
  const downloadTemplate = (e: React.MouseEvent) => {
    ...
    const templateData = [
      ...
      ['Software shall validate input data range before processing', 'Software', 'SYSR-001', 'HAZ-SWR-001', 'Not Started', ''],
      ['Software shall log all user actions with timestamps', 'Software', 'SYSR-002', '', 'Not Started', ''],
      ['Software shall display error messages within 500ms', 'Usability', 'SYSR-003', 'HAZ-SWR-002', 'Not Started', 'DHS-UI-100'],
      ['Software shall encrypt data at rest using AES-256', 'Safety', 'SYSR-004', '', 'Not Started', ''],
      ['Software shall support firmware update via USB', 'Software', 'SYSR-005', '', 'Not Started', 'DHS-MCU-200'],
    ];
    ...
    const instructionsData = [
      ...
      ['Description', 'Yes', 'The software requirement statement', 'Any text'],
      ...
      ['Linked Risks', 'No', 'Hazard IDs mitigated by this requirement (comma-separated)', 'e.g. HAZ-SWR-001'],
      ['Verification Status', 'No', 'Current verification state (defaults to Not Started)', 'Not Started, In Progress, Passed, Failed'],
      ['Linked Component Part Number', 'No', 'Part number or name of linked device component', 'e.g. DHS-MCU-200'],
    ];
    ...
    XLSX.writeFile(wb, 'software-requirements-import-template.xlsx');
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ['requirement-specifications', productId, 'software'] });
      toast.success(`Imported ${succeeded} software requirements${linked > 0 ? `, ${linked} linked to components` : ''}`);
      handleClose(false);
    } catch (err: any) {
      console.error('SWR import failed:', err);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh]">
        ...
        {step === 'upload' && (
          <div
            ...
            onDrop={handleDrop}
            onClick={() => document.getElementById('swr-import-file')?.click()}
          >
            ...
            <input id="swr-import-file" type="file" accept=".csv,.xlsx,.xls" className="hidden" onChange={handleFileInput} />
          </div>
        )}
        ...
        {step === 'import' && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Each requirement will receive an auto-generated SWR-XXX ID. Requirements are inserted sequentially to ensure unique IDs.
            </p>
          </div>
        )}
