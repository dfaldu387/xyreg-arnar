...
import { HardwareRequirementsAIService, HardwareRequirementSuggestion } from "@/services/hardwareRequirementsAIService";
...
interface HardwareAISuggestionsDialogProps {
  open: boolean;
...
export function HardwareAISuggestionsDialog({
  open,
  onOpenChange,
  productId,
  companyId
}: HardwareAISuggestionsDialogProps) {
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (results) => {
      toast.success(lang('hardwareRequirements.suggestions.importSuccess'));
    ...
    onError: (error) => {
      toast.error(lang('hardwareRequirements.suggestions.importError'));
      ...
      <DialogContent className="max-w-xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {lang('hardwareRequirements.suggestions.title')}
          </DialogTitle>
          <DialogDescription>
            {lang('hardwareRequirements.suggestions.description')}
          </DialogDescription>
        </DialogHeader>
          ...
          {suggestions.length === 0 ? (
            <div className="flex-1 overflow-y-auto flex flex-col items-center gap-4 py-4">
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  <h3 className="text-lg font-medium">{lang('hardwareRequirements.suggestions.generateTitle')}</h3>
                  <p className="text-muted-foreground">
                    {lang('hardwareRequirements.suggestions.generatePrompt')}
                  </p>
                </div>
                <Button
                ...
                >
                  {isGenerating || generateMutation.isPending ? (
                    <>
                      ...
                      {lang('hardwareRequirements.suggestions.generating')}
                    </>
                  ) : (
                    <>
                      <Sparkles className="h-4 w-4 mr-2" />
                      {lang('hardwareRequirements.suggestions.generateButton')}
                    </>
                  )}
                </Button>
              </div>
            ...
            <>
              <div className="flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">
                    {lang('hardwareRequirements.suggestions.suggestionsCount', { count: suggestions.length })}
                  </span>
                  <Badge variant="outline">
                    {lang('hardwareRequirements.suggestions.selectedCount', { count: selectedSuggestions.size })}
                  </Badge>
                </div>
                <div className="flex gap-2">
                  <Button variant="ghost" size="sm" onClick={selectAll}>
                    {lang('hardwareRequirements.suggestions.selectAll')}
                  </Button>
                  <Button variant="ghost" size="sm" onClick={selectNone}>
                    {lang('hardwareRequirements.suggestions.selectNone')}
                  </Button>
                  <Button
                    onClick={() => generateMutation.mutate()}
                    disabled={isGenerating || generateMutation.isPending}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <div className="text-sm text-muted-foreground space-y-1">
                            <div>
                              <span className="font-medium">{lang('hardwareRequirements.suggestions.rationale')}:</span> {suggestion.rationale}
                            </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t">
                ...
                >
                  ...
                  {lang('hardwareRequirements.suggestions.importButton', { count: selectedSuggestions.size })}
                </Button>
              </div>
            </>
          )}
