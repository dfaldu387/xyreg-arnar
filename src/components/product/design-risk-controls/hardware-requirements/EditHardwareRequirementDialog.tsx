...
interface EditHardwareRequirementDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  requirement: RequirementSpecification | null;
  onSave: (id: string, updates: { description: string; category: string; traces_to: string; linked_risks: string; verification_status: string; component_id?: string | null }) => Promise<void>;
...
export function EditHardwareRequirementDialog({
  open,
  onOpenChange,
  requirement,
  onSave,
  ...
  disabled = false,
  productId,
}: EditHardwareRequirementDialogProps) {
  ...
  const { data: affectedLinks = [] } = useQuery({
    queryKey: ['affected-links-for-hwr', requirement?.requirement_id],
    queryFn: async () => {
  ...
  React.useEffect(() => {
    if (requirement) {
      ...
      setComponentId(requirement.component_id || '');
      setSelectedRisks(
  ...
  const triggerAiSuggestion = async () => {
    ...
    setAiSuggested(false);
    try {
      const categories = HWR_CATEGORIES.map(c => ({ id: c.id, label: c.label, description: c.label }));
      const { data, error } = await supabase.functions.invoke('ai-category-suggester', {
        body: { description: formData.description, categories, context: 'hardware-requirement' }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onSave(requirement.id, { ...formData, linked_risks: selectedRisks.join(', '), component_id: componentId || null });
    ...
    <>
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Edit Hardware Requirement - {requirement.requirement_id}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Select value={componentId} onValueChange={setComponentId} disabled={disabled}>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    navigate(`/app/product/${routeProductId || productId}/design-risk-controls?tab=risk-management&returnTo=hardware-requirements`);
                  } catch (e) {
          ...
          {affectedLinks.length > 0 && (
            <div className="p-3 border border-orange-200 bg-orange-50 rounded-lg">
              <p className="text-sm font-medium text-orange-800 mb-2">
                The following requirements reference this hardware requirement:
              </p>
