...
interface HardwareRequirementsTableProps {
...
export function HardwareRequirementsTable({ requirements, disabled = false, onEditRequirement, validSystemReqIds, productId }: HardwareRequirementsTableProps) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: string) => {
        ...
        const comp = componentMap.get(value);
        ...
        return (
          <Badge variant="outline" className="bg-accent/50 text-accent-foreground border-accent text-xs">
            ...
            {comp.part_number ? `${comp.part_number}` : comp.name}
          </Badge>
        );
      },
    },
              ...
              return (
                <Badge
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    if (!isMissing) {
                      navigate(`${location.pathname}?tab=requirement-specifications&subTab=system-requirements&returnTo=hardware-requirements`);
                    }
                  }}
                >
          ...
          <div className="flex flex-wrap gap-1">
            {ids.map((id: string) => (
              <Badge
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(`${location.pathname}?tab=risk-management&subTab=hazard-traceability-matrix&returnTo=hardware-requirements`);
                }}
              >
                {id}
              </Badge>
