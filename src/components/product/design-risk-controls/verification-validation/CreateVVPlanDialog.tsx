...
interface CreateVVPlanDialogProps {
  open: boolean;
...
export function CreateVVPlanDialog({
  open,
  onOpenChange,
  productId,
  companyId,
  ...
  // Pre-fill form when initialData or editPlanId changes
  React.useEffect(() => {
    if (open && initialData) {
      form.reset({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{editPlanId ? "Edit V&V Plan" : lang("verificationValidation.vvPlan.createDialog.title")}</DialogTitle>
          <DialogDescription>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Scope info (read-only, determined by module toggle) */}
