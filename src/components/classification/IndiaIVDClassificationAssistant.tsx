...
import { useIndiaIVDClassificationAssistant } from '@/hooks/useIndiaIVDClassificationAssistant';
import { ClassificationResult } from '@/types/classification';

interface IndiaIVDClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete?: (classification: string, result: ClassificationResult) => void;
}

export function IndiaIVDClassificationAssistant({
  isOpen,
  onClose,
  onComplete
}: IndiaIVDClassificationAssistantProps) {
  const {
    session,
    currentQuestion,
    selectOption,
    ...
    canGoBack,
    isComplete
  } = useIndiaIVDClassificationAssistant();
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            India IVD Classification Assistant
          </DialogTitle>
          <DialogDescription>
            Determine the IVD classification based on CDSCO IVDR 2017 (Classes A, B, C, D)
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <Card className="bg-muted/50">
                <CardContent className="py-3">
                  <a
                    href="https://cdsco.gov.in/opencms/opencms/en/Medical-Devices/Medical-Devices/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm text-primary hover:underline flex items-center gap-1"
                  >
                    <ExternalLink className="h-3 w-3" />
                    CDSCO IVD Regulations
                  </a>
                </CardContent>
              </Card>
            </div>
