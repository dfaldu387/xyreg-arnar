...
import { useChinaClassificationAssistant } from "@/hooks/useChinaClassificationAssistant";
import { cn } from "@/lib/utils";
import { GenericRuleTextDialog } from './GenericRuleTextDialog';
import { getRuleText } from '@/data/chinaRuleTexts';

interface ChinaClassificationTriggerProps {
...
export function ChinaClassificationTrigger({
  onClassificationSelected,
  isOpen: externalIsOpen,
  onOpenChange,
  showButton = true
}: ChinaClassificationTriggerProps) {
  ...
  const { session, currentQuestion, selectOption, goBack, restart, canGoBack, isComplete } = useChinaClassificationAssistant();

  const handleRuleClick = (ruleText: string) => {
    // Extract from format like "NMPA Classification - Class III"
    // China uses class-based rules
    if (ruleText.includes('Class I') && !ruleText.includes('Class II') && !ruleText.includes('Class III')) {
      setSelectedRuleId('rule_class1');
    } else if (ruleText.includes('Class II')) {
      setSelectedRuleId('rule_class2');
    } else if (ruleText.includes('Class III')) {
    ...
    <>
      {showButton && (
        ...
        >
          ...
          China NMPA Classification Assistant
        </Button>
      )}
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="mt-6 p-3 bg-muted/50 rounded-md border border-border/50">
            <p className="text-xs text-muted-foreground">
              <span className="font-semibold">Disclaimer:</span> This assistant provides guidance based on China NMPA regulations but xyreg assumes no responsibility for the accuracy of classifications. Always consult with qualified regulatory experts for final validation. This tool is for informational purposes only.
            </p>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <GenericRuleTextDialog
        ...
        marketName="China NMPA"
        regulationDescription="China NMPA Medical Device Supervision Regulations"
        officialUrl="https://www.nmpa.gov.cn/"
        officialLinkText="View Official NMPA Website"
      />
    </>
  );
}
