...
import { useIndiaSaMDClassificationAssistant } from '@/hooks/useIndiaSaMDClassificationAssistant';

interface IndiaSaMDClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onClassificationComplete: (deviceClass: DeviceClass, result?: ClassificationResult) => void;
}

export function IndiaSaMDClassificationAssistant({
  isOpen,
  onClose,
  onClassificationComplete
}: IndiaSaMDClassificationAssistantProps) {
  const {
    currentQuestion,
    selectOption,
    goBack,
    ...
    isComplete,
    session
  } = useIndiaSaMDClassificationAssistant();
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            CDSCO Software Classification Assistant
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
            ...
            <Card className={`border-2 ${getClassColor(session.result.class)}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  CDSCO Software Classification Result
                </CardTitle>
                <CardDescription>
                  Based on your answers, here is the recommended classification under CDSCO Medical Devices Rules 2017:
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-orange-700 space-y-1">
                    {session.result.class === 'Class D' && (
                      <>
                        <li>• CDSCO registration required (pre-market approval)</li>
                        <li>• Clinical investigation in India required</li>
                        <li>• Indian Authorized Agent required for foreign manufacturers</li>
                        <li>• ISO 13485 certification required</li>
                        <li>• Import license required</li>
                      </>
                    )}
                    {session.result.class === 'Class C' && (
                      <>
                        <li>• CDSCO registration required</li>
                        <li>• Clinical evidence required</li>
                        <li>• Indian Authorized Agent required</li>
                        <li>• ISO 13485 certification required</li>
                      </>
                    )}
                    {session.result.class === 'Class B' && (
                      <>
                        <li>• CDSCO registration required</li>
                    ...
                    {session.result.class === 'Class A' && (
                      <>
                        <li>• CDSCO notification/registration</li>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-amber-700 space-y-1">
                    ...
                    <li>• CDSCO Software guidance compliance</li>
                    <li>• Cybersecurity requirements</li>
                    <li>• Clinical evaluation for Class C/D</li>
                  </ul>
                </div>

                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <p className="text-sm text-red-700">
                    <strong>Disclaimer:</strong> This classification is for guidance only. Always verify requirements with CDSCO and consult with an Indian regulatory expert.
                  </p>
                </div>
              ...
              <CardHeader>
                ...
                {currentQuestion.helpText && (
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                    <p className="text-sm text-orange-700">{currentQuestion.helpText}</p>
                  </div>
                )}
              </CardHeader>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm text-muted-foreground">
              Question {session.path.length} of CDSCO Classification
            </div>
          </div>
        </div>
      </DialogContent>
