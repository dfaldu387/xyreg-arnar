...
import { useBrazilIVDClassificationAssistant } from '@/hooks/useBrazilIVDClassificationAssistant';
import { ClassificationResult } from '@/types/classification';

interface BrazilIVDClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete?: (classification: string, result: ClassificationResult) => void;
}

export function BrazilIVDClassificationAssistant({
  isOpen,
  onClose,
  onComplete
}: BrazilIVDClassificationAssistantProps) {
  const {
    session,
    currentQuestion,
    selectOption,
    ...
    canGoBack,
    isComplete
  } = useBrazilIVDClassificationAssistant();
  ...
  const getClassBadgeVariant = (classification: string) => {
    if (classification.includes('IV')) return 'destructive';
    if (classification.includes('III')) return 'default';
    if (classification.includes('II')) return 'secondary';
    return 'outline';
  };
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            Brazil IVD Classification Assistant
          </DialogTitle>
          <DialogDescription>
            Determine the IVD classification based on ANVISA RDC 36/2015 (Classes I-IV)
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <Card className="bg-muted/50">
                <CardContent className="py-3">
                  <a
                    href="https://www.gov.br/anvisa/pt-br/assuntos/regulamentacao/legislacao/bibliotecas-tematicas/arquivos/produtos-para-a-saude"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm text-primary hover:underline flex items-center gap-1"
                  >
                    <ExternalLink className="h-3 w-3" />
                    ANVISA IVD Regulations
                  </a>
                </CardContent>
              </Card>
            </div>
