...
import { useFDAIVDClassificationAssistant } from '@/hooks/useFDAIVDClassificationAssistant';
import { ClassificationResult } from '@/types/classification';

interface FDAIVDClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete?: (classification: string, result: ClassificationResult) => void;
}

export function FDAIVDClassificationAssistant({
  isOpen,
  onClose,
  onComplete
}: FDAIVDClassificationAssistantProps) {
  const {
    session,
    currentQuestion,
    selectOption,
    ...
    canGoBack,
    isComplete
  } = useFDAIVDClassificationAssistant();
  ...
  const getClassBadgeVariant = (classification: string) => {
    if (classification.includes('III')) return 'destructive';
    if (classification.includes('II')) return 'default';
    return 'secondary';
  };
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            FDA IVD Classification Assistant
          </DialogTitle>
          <DialogDescription>
            Determine the FDA IVD classification based on 21 CFR Part 809 (Classes I, II, III)
          </DialogDescription>
        </DialogHeader>
              ...
              <Card className="border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/30">
                ...
                <CardContent className="space-y-3">
                  ...
                  {session.result.productCodeExamples && session.result.productCodeExamples.length > 0 && (
                    <div>
                      <h4 className="font-medium text-sm text-muted-foreground">Example Product Codes</h4>
