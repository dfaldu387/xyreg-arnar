...
import { useSouthKoreaIVDClassificationAssistant } from '@/hooks/useSouthKoreaIVDClassificationAssistant';
import { ClassificationResult } from '@/types/classification';

interface SouthKoreaIVDClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete?: (classification: string, result: ClassificationResult) => void;
}

export function SouthKoreaIVDClassificationAssistant({
  isOpen,
  onClose,
  onComplete
}: SouthKoreaIVDClassificationAssistantProps) {
  const {
    session,
    currentQuestion,
    selectOption,
    ...
    canGoBack,
    isComplete
  } = useSouthKoreaIVDClassificationAssistant();
  ...
  const getClassBadgeVariant = (classification: string) => {
    if (classification.includes('IV') || classification.includes('4')) return 'destructive';
    if (classification.includes('III') || classification.includes('3')) return 'default';
    if (classification.includes('II') || classification.includes('2')) return 'secondary';
    return 'outline';
  };
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            South Korea IVD Classification Assistant
          </DialogTitle>
          <DialogDescription>
            Determine the IVD classification based on MFDS regulations (Classes 1-4)
          </DialogDescription>
        </DialogHeader>
