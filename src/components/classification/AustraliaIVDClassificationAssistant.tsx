...
import { useAustraliaIVDClassificationAssistant } from '@/hooks/useAustraliaIVDClassificationAssistant';
import { ClassificationResult } from '@/types/classification';

interface AustraliaIVDClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete?: (classification: string, result: ClassificationResult) => void;
}

export function AustraliaIVDClassificationAssistant({
  isOpen,
  onClose,
  onComplete
}: AustraliaIVDClassificationAssistantProps) {
  const {
    session,
    currentQuestion,
    selectOption,
    ...
    canGoBack,
    isComplete
  } = useAustraliaIVDClassificationAssistant();
  ...
  const getClassBadgeVariant = (classification: string) => {
    if (classification.includes('4')) return 'destructive';
    if (classification.includes('3')) return 'default';
    if (classification.includes('2')) return 'secondary';
    return 'outline';
  };
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            Australia IVD Classification Assistant
          </DialogTitle>
          <DialogDescription>
            Determine the IVD classification based on TGA regulations (Classes 1-4)
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <Card className="bg-muted/50">
                <CardContent className="py-3">
                  <a
                    href="https://www.tga.gov.au/how-we-regulate/manufacturing/manufacture-medical-devices/manufacture-ivd-medical-devices"
                    target="_blank"
