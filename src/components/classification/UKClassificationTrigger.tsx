...
import { useUKClassificationAssistant } from "@/hooks/useUKClassificationAssistant";
import { cn } from "@/lib/utils";
import { GenericRuleTextDialog } from './GenericRuleTextDialog';
import { getRuleByNumber } from '@/data/ukRuleTexts';

interface UKClassificationTriggerProps {
...
export function UKClassificationTrigger({ 
  onClassificationSelected,
  isOpen: externalIsOpen,
  onOpenChange,
  showButton = true
}: UKClassificationTriggerProps) {
  ...
  const { session, currentQuestion, selectOption, goBack, restart, canGoBack, isComplete } = useUKClassificationAssistant();
  ...
  const handleComplete = () => {
    if (session.result) {
      // Map UK classes to internal format
      let riskClass = '';
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              UK MHRA Classification Assistant
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="mt-6 p-3 bg-muted/50 rounded-md border border-border/50">
            <p className="text-xs text-muted-foreground">
              <span className="font-semibold">Disclaimer:</span> This assistant provides guidance based on UK MHRA Medical Devices Regulations but xyreg assumes no responsibility for the accuracy of classifications. Always consult with qualified regulatory experts for final validation. This tool is for informational purposes only.
            </p>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <GenericRuleTextDialog
        ...
        marketName="UK MHRA"
        regulationDescription="UK Medical Devices Regulations 2002 (as amended)"
        officialUrl="https://www.legislation.gov.uk/uksi/2002/618"
        officialLinkText="View Official legislation.gov.uk Source"
      />
    </>
  );
}
