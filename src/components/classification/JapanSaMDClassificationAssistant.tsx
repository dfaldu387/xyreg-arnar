...
import { useJapanSaMDClassificationAssistant } from '@/hooks/useJapanSaMDClassificationAssistant';

interface JapanSaMDClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onClassificationComplete: (deviceClass: DeviceClass, result?: ClassificationResult) => void;
}

export function JapanSaMDClassificationAssistant({
  isOpen,
  onClose,
  onClassificationComplete
}: JapanSaMDClassificationAssistantProps) {
  const {
    currentQuestion,
    selectOption,
    goBack,
    ...
    isComplete,
    session
  } = useJapanSaMDClassificationAssistant();
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            PMDA Software Classification Assistant
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
            ...
            <Card className={`border-2 ${getClassColor(session.result.class)}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  PMDA Software Classification Result
                </CardTitle>
                <CardDescription>
                  Based on your answers, here is the recommended classification under Japan's PMD Act:
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <h4 className="font-semibold mb-2 text-red-800 flex items-center gap-2">
                    ...
                    PMDA Regulatory Requirements
                  </h4>
                  <ul className="text-sm text-red-700 space-y-1">
                    {session.result.class === 'Class IV' && (
                      <>
                        <li>• PMDA Pre-market Approval (Shonin) required</li>
                        <li>• Clinical trials in Japan typically required</li>
                        <li>• QMS (Quality Management System) certification</li>
                        <li>• Marketing Authorization Holder (MAH) in Japan required</li>
                        <li>• Japanese language documentation</li>
                      </>
                    )}
                    {session.result.class === 'Class III' && (
                      <>
                        <li>• PMDA Certification (Ninsho) through RCB required</li>
                        <li>• QMS certification required</li>
                        <li>• Marketing Authorization Holder (MAH) in Japan</li>
                        <li>• Clinical evidence required</li>
                      </>
                    )}
                    {session.result.class === 'Class II' && (
                      <>
                        <li>• PMDA Notification (Todokede) required</li>
                        <li>• QMS compliance required</li>
                        <li>• Marketing Authorization Holder (MAH) in Japan</li>
                      </>
                    )}
                    {session.result.class === 'Class I' && (
                      <>
                        <li>• Basic notification requirements</li>
                        <li>• Marketing Authorization Holder (MAH) in Japan</li>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-amber-700 space-y-1">
                    ...
                    <li>• JIS T 2304: Japanese standard for software</li>
                    <li>• PMDA SaMD guidance compliance</li>
                    <li>• Cybersecurity requirements</li>
                    <li>• Japanese language user interface (if patient-facing)</li>
                  </ul>
                </div>

                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <p className="text-sm text-red-700">
                    <strong>Disclaimer:</strong> This classification is for guidance only. PMDA requirements are specific to Japan. Always consult with a Japanese regulatory expert.
                  </p>
                </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm text-muted-foreground">
              Question {session.path.length} of PMDA Classification
            </div>
          </div>
        </div>
      </DialogContent>
