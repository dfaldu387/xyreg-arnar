...
import { AustraliaIVDClassificationAssistant } from './AustraliaIVDClassificationAssistant';
import { ClassificationResult } from '@/types/classification';

interface AustraliaIVDClassificationTriggerProps {
  onClassificationSelected?: (riskClass: string, result: ClassificationResult) => void;
  onOpenChange?: (isOpen: boolean) => void;
  showButton?: boolean;
}

export function AustraliaIVDClassificationTrigger({
  onClassificationSelected,
  onOpenChange,
  showButton = true
}: AustraliaIVDClassificationTriggerProps) {
  ...
  const handleClassificationComplete = (classification: string, result: ClassificationResult) => {
    if (onClassificationSelected) {
      // Normalize to Australia IVD format: "1", "2", "3", "4" (numeric)
    ...
    <>
      {showButton && (
        ...
        >
          ...
          Australia IVD Assistant
        </Button>
      )}

      <AustraliaIVDClassificationAssistant
        isOpen={isOpen}
