...
import { useUKIVDRClassificationAssistant } from '@/hooks/useUKIVDRClassificationAssistant';
import { ClassificationResult } from '@/types/classification';

interface UKIVDRClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete?: (classification: string, result: ClassificationResult) => void;
}

export function UKIVDRClassificationAssistant({
  isOpen,
  onClose,
  onComplete
}: UKIVDRClassificationAssistantProps) {
  const {
    session,
    currentQuestion,
    selectOption,
    ...
    canGoBack,
    isComplete
  } = useUKIVDRClassificationAssistant();
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            UK IVDR Classification Assistant
          </DialogTitle>
          <DialogDescription>
            Determine the UK IVD classification based on MHRA requirements (Classes A-D)
          </DialogDescription>
        </DialogHeader>
