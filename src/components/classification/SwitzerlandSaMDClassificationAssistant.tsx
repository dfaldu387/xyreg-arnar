...
import { useSwitzerlandSaMDClassificationAssistant } from '@/hooks/useSwitzerlandSaMDClassificationAssistant';

interface SwitzerlandSaMDClassificationAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  onClassificationComplete: (deviceClass: DeviceClass, result?: ClassificationResult) => void;
}

export function SwitzerlandSaMDClassificationAssistant({
  isOpen,
  onClose,
  onClassificationComplete
}: SwitzerlandSaMDClassificationAssistantProps) {
  const {
    currentQuestion,
    selectOption,
    goBack,
    ...
    isComplete,
    session
  } = useSwitzerlandSaMDClassificationAssistant();
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Swissmedic Software Classification Assistant
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
            ...
            <Card className={`border-2 ${getClassColor(session.result.class)}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Swissmedic Software Classification Result
                </CardTitle>
                <CardDescription>
                  Based on your answers, here is the recommended classification under Swiss MedDO:
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-red-700 space-y-1">
                    <li>• Swissmedic registration required</li>
                    <li>• Swiss Authorized Representative (CH-REP) required for foreign manufacturers</li>
                    <li>• Declaration of Conformity</li>
                    <li>• CE marking (recognized by Swissmedic)</li>
                    <li>• Conformity assessment by EU Notified Body accepted</li>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-amber-700 space-y-1">
                    ...
                    <li>• MedDO aligned with EU MDR Rule 11</li>
                    <li>• Cybersecurity requirements</li>
                    <li>• Clinical evaluation required</li>
                  </ul>
                </div>

                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="font-semibold mb-2 text-blue-800">MRA Agreement Note:</h4>
                  <p className="text-sm text-blue-700">
                    Switzerland has a Mutual Recognition Agreement (MRA) with the EU. CE marked devices with conformity assessment from EU Notified Bodies are generally recognized in Switzerland, but registration with Swissmedic is still required.
                  </p>
                </div>

                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <p className="text-sm text-red-700">
                    <strong>Disclaimer:</strong> This classification is for guidance only. Swiss regulations may differ from EU MDR. Always verify current requirements with Swissmedic.
                  </p>
                </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm text-muted-foreground">
              Question {session.path.length} of Swissmedic Classification
            </div>
          </div>
        </div>
      </DialogContent>
