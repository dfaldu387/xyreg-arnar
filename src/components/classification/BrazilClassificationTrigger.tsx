...
import { useBrazilClassificationAssistant } from "@/hooks/useBrazilClassificationAssistant";
import { cn } from "@/lib/utils";
import { GenericRuleTextDialog } from './GenericRuleTextDialog';
import { getRuleText } from '@/data/brazilRuleTexts';

interface BrazilClassificationTriggerProps {
...
export function BrazilClassificationTrigger({
  onClassificationSelected,
  isOpen: externalIsOpen,
  onOpenChange,
  showButton = true
}: BrazilClassificationTriggerProps) {
  ...
  const { session, currentQuestion, selectOption, goBack, restart, canGoBack, isComplete } = useBrazilClassificationAssistant();

  const handleRuleClick = (ruleText: string) => {
    // Extract rule number from format like "RDC 185/2001 - Rule 6"
    const ruleMatch = ruleText.match(/Rule\s+(\d+)/i);
    if (ruleMatch) {
      const ruleNum = ruleMatch[1];
      // Brazil rules 1-3 map to Classes I-II, 4-6 to Class III, 7-9 to Class IV
      if (ruleNum === '1') setSelectedRuleId('rule_class1');
      else if (['2', '3'].includes(ruleNum)) setSelectedRuleId('rule_class2');
      else if (['4', '5', '6'].includes(ruleNum)) setSelectedRuleId('rule_class3');
      else if (['7', '8', '9'].includes(ruleNum)) setSelectedRuleId('rule_class4');
    ...
    <>
      {showButton && (
        ...
        >
          ...
          Brazil ANVISA Classification Assistant
        </Button>
      )}
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="mt-6 p-3 bg-muted/50 rounded-md border border-border/50">
            <p className="text-xs text-muted-foreground">
              <span className="font-semibold">Disclaimer:</span> This assistant provides guidance based on Brazil ANVISA regulations but xyreg assumes no responsibility for the accuracy of classifications. Always consult with qualified regulatory experts for final validation. This tool is for informational purposes only.
            </p>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <GenericRuleTextDialog
        ...
        marketName="Brazil ANVISA"
        regulationDescription="Brazilian ANVISA RDC 185/2001 Medical Device Regulations"
        officialUrl="https://www.gov.br/anvisa/pt-br"
        officialLinkText="View Official ANVISA Website"
      />
    </>
  );
}
