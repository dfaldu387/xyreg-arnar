      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="lg:col-span-1">
          <CardContent className="pt-5 pb-4 px-5">
            <h2 className="text-base font-semibold mb-4">Usage by Feature</h2>
                ...
                {orgSummaries.map(s => (
                  <TableRow key={s.companyId} className="hover:bg-muted/40">
                    ...
                    <TableCell className="text-center">
                      <button className="text-sm text-primary underline hover:text-primary/80" onClick={() => setDetailOrg(s)}>
                        View
                      </button>
                    </TableCell>
                  </TableRow>
            ...
            <div className="space-y-4 overflow-y-auto max-h-[calc(85vh-80px)] pr-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                {Object.entries(detailOrg.bySource).map(([source, data]) => (
                  <div key={source} className="rounded-lg border p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <div className="w-2 h-2 rounded-full" style={{ background: SOURCE_COLORS[source] || '#94a3b8' }} />
                      <span className="text-sm font-medium">{SOURCE_LABELS[source] || source}</span>
                    </div>
                    <p className="text-lg font-bold">{formatTokens(data.tokens)}</p>
                    <p className="text-xs text-muted-foreground">{data.calls} calls &middot; ${estimateCost(data.inputTokens, data.outputTokens, data.thinkingTokens).toFixed(4)}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="border rounded-lg overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {detailOrg.rows.slice(0, 50).map(r => (
                      <TableRow key={r.id}>
                ...
                {detailOrg.rows.length > 50 && (
                  <div className="px-4 py-2 text-xs text-muted-foreground border-t">
                    Showing 50 of {detailOrg.rows.length} requests
                  </div>
                )}
              </div>
            </div>
