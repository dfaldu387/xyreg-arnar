...
import { brazilSaMDClassificationQuestions } from '@/data/brazilSaMDClassificationRules';

export function useBrazilSaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'br_samd_initial',
    answers: {},
    path: ['br_samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return brazilSaMDClassificationQuestions[session.currentQuestionId] || null;
  };
