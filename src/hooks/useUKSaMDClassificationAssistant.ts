...
import { ukSaMDClassificationQuestions } from '@/data/ukSaMDClassificationRules';

export function useUKSaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'uk_samd_initial',
    answers: {},
    path: ['uk_samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return ukSaMDClassificationQuestions[session.currentQuestionId] || null;
  };
