...
import { indiaIVDClassificationQuestions } from '@/data/indiaIVDClassificationRules';

export function useIndiaIVDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({ currentQuestionId: 'india_ivd_initial', answers: {}, path: ['india_ivd_initial'] });
  const getCurrentQuestion = (): Question | null => indiaIVDClassificationQuestions[session.currentQuestionId] || null;
  ...
  const restart = () => setSession({ currentQuestionId: 'india_ivd_initial', answers: {}, path: ['india_ivd_initial'] });
