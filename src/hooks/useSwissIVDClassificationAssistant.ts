...
import { swissIVDClassificationQuestions } from '@/data/swissIVDClassificationRules';

export function useSwissIVDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'swiss_ivd_initial',
    answers: {},
    path: ['swiss_ivd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return swissIVDClassificationQuestions[session.currentQuestionId] || null;
  };
