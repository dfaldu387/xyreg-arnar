...
import { indiaSaMDClassificationQuestions } from '@/data/indiaSaMDClassificationRules';

export function useIndiaSaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'in_samd_initial',
    answers: {},
    path: ['in_samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return indiaSaMDClassificationQuestions[session.currentQuestionId] || null;
  };
