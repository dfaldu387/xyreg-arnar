...
import { fdaSaMDClassificationQuestions } from '@/data/fdaSaMDClassificationRules';

export function useFDASaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'fda_samd_initial',
    answers: {},
    path: ['fda_samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return fdaSaMDClassificationQuestions[session.currentQuestionId] || null;
  };
