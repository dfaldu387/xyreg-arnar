...
import { samdClassificationQuestions } from '@/data/samdClassificationRules';

export function useSaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'samd_initial',
    answers: {},
    path: ['samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return samdClassificationQuestions[session.currentQuestionId] || null;
  };
