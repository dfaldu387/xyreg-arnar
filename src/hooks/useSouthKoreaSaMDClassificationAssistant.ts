...
import { southKoreaSaMDClassificationQuestions } from '@/data/southKoreaSaMDClassificationRules';

export function useSouthKoreaSaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'kr_samd_initial',
    answers: {},
    path: ['kr_samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return southKoreaSaMDClassificationQuestions[session.currentQuestionId] || null;
  };
