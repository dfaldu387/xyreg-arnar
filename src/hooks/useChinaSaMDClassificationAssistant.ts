...
import { chinaSaMDClassificationQuestions } from '@/data/chinaSaMDClassificationRules';

export function useChinaSaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'cn_samd_initial',
    answers: {},
    path: ['cn_samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return chinaSaMDClassificationQuestions[session.currentQuestionId] || null;
  };
