...
import { japanSaMDClassificationQuestions } from '@/data/japanSaMDClassificationRules';

export function useJapanSaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'jp_samd_initial',
    answers: {},
    path: ['jp_samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return japanSaMDClassificationQuestions[session.currentQuestionId] || null;
  };
