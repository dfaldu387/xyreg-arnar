...
import { canadaIVDClassificationQuestions } from '@/data/canadaIVDClassificationRules';

export function useCanadaIVDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'canada_ivd_initial',
    answers: {},
    path: ['canada_ivd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return canadaIVDClassificationQuestions[session.currentQuestionId] || null;
  };
