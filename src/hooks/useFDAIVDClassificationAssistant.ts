...
import { fdaIVDClassificationQuestions } from '@/data/fdaIVDClassificationRules';

export function useFDAIVDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'fda_ivd_initial',
    answers: {},
    path: ['fda_ivd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return fdaIVDClassificationQuestions[session.currentQuestionId] || null;
  };
