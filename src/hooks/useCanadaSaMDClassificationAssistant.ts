...
import { canadaSaMDClassificationQuestions } from '@/data/canadaSaMDClassificationRules';

export function useCanadaSaMDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({
    currentQuestionId: 'ca_samd_initial',
    answers: {},
    path: ['ca_samd_initial']
  });

  const getCurrentQuestion = (): Question | null => {
    return canadaSaMDClassificationQuestions[session.currentQuestionId] || null;
  };
