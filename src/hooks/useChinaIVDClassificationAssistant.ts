...
import { chinaIVDClassificationQuestions } from '@/data/chinaIVDClassificationRules';

export function useChinaIVDClassificationAssistant() {
  const [session, setSession] = useState<ClassificationSession>({ currentQuestionId: 'china_ivd_initial', answers: {}, path: ['china_ivd_initial'] });
  const getCurrentQuestion = (): Question | null => chinaIVDClassificationQuestions[session.currentQuestionId] || null;
  ...
  const restart = () => setSession({ currentQuestionId: 'china_ivd_initial', answers: {}, path: ['china_ivd_initial'] });
