...
export class DocToPdfConverterService {
  ...
  static async convertDocxToPdfWithHeader(
    ...
    try {
      ...
      const pdfMake = await initPdfMake();
      const meta = (options as any).pageMeta || {};
      const isDraft = !!meta.isDraft;
      const runningHeaderText = meta.runningHeader || '';
      ...
      const docDefinition = {
        ...
        header: runningHeaderText
          ? (currentPage: number) => ({
              columns: [
                { text: runningHeaderText, fontSize: 8, color: '#666666', margin: [40, 30, 0, 0] },
              ],
            })
          : undefined,
