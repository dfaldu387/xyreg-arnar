...
export class DocumentPdfPreviewService {
  ...
  static async generatePreviewPdfBlob(
    ...
    // Build per-page running metadata (header/footer/watermark)
    const dc = (studioDocControl || {}) as Record<string, any>;
    const ca = (dcAssignment?.control_data as Record<string, any>) || {};
    const docTypeLabel = dc.documentType || ca.documentType || padtRow?.document_reference?.split('-')[0] || 'Document';
    const sopNumberMeta = dc.sopNumber || ca.sopNumber || padtRow?.document_reference || '';
    const versionMeta = dc.version || ca.version || '';
    const lifecycleState = (dc.lifecycleState || ca.lifecycleState || '').toString().toLowerCase();
    const isDraft = lifecycleState !== 'effective' && lifecycleState !== 'released';

    const { data: { user } } = await supabase.auth.getUser();
    const generatedBy = user?.user_metadata?.full_name || user?.email || '';
    const generatedAtUtc = format(new Date(), "dd MMM yyyy 'at' HH:mm:ss") + ' (UTC)';

    const runningHeaderParts = [
      docTypeLabel,
      companyName,
      documentName,
      sopNumberMeta,
    ...
    const runningHeader = runningHeaderParts.join(' · ');

    const pageMeta = { isDraft, runningHeader, generatedBy, generatedAtUtc };
    ...
    if (reviewFilePath) {
      ...
      const pdfBlob = await DocToPdfConverterService.convertDocxToPdfWithHeader(
        docxBlob,
        headerContent,
        { pageMeta } as any
      );
   ...
   */
  private static async generateHeaderOnlyPdf(
    content: ContentItem[],
    pageMeta?: { isDraft?: boolean; runningHeader?: string; generatedBy?: string; generatedAtUtc?: string }
  ): Promise<Blob | null> {
    try {
      ...
      return new Promise((resolve, reject) => {
        try {
          const meta = pageMeta || {};
          const runningHeader = meta.runningHeader || '';
          const generatedBy = meta.generatedBy || '';
          const generatedAtUtc = meta.generatedAtUtc || '';
          const isDraft = !!meta.isDraft;
          const docDefinition = {
            content,
            defaultStyle: { font: 'Roboto', fontSize: 11, lineHeight: 1.4 },
            pageSize: 'A4' as const,
            pageMargins: [40, 80, 40, 90] as [number, number, number, number],
                  ...
                  columns: [
                    {
                      text: [
                        ...
                        generatedAtUtc ? `on ${generatedAtUtc} ` : '',
                        'Uncontrolled copy. Valid for 24 hours only.',
                      ],
                      fontSize: 7,
                      color: '#666666',
                      width: '*',
  ...
  private static buildPdfMakeHeader(
    ...
    const documentAuthor = dc.documentAuthor || ca.documentAuthor || '';
    const approvalDate = dc.approvalDate ? fmtDate(dc.approvalDate) : '';
    const nextReviewDate = dc.nextPeriodicReviewDate
      ? fmtDate(dc.nextPeriodicReviewDate)
      : (dc.reviewPeriod ? `${dc.reviewPeriod} months after effective` : 'N/A');
    const supersededDate = dc.supersededDate ? fmtDate(dc.supersededDate) : '';
    const documentType = dc.documentType || ca.documentType || '';
    const templateVersion = dc.templateVersion || ca.templateVersion || '';
    const documentTypeLabel = documentType
      ? (templateVersion ? `${documentType} (${templateVersion})` : documentType)
      : '';
    ...
    if (documentTypeLabel) infoRows.push(makeInfoRow('Document Type', documentTypeLabel));
    if (version) infoRows.push(makeInfoRow('Version', version));
    if (documentAuthor) infoRows.push(makeInfoRow('Author', documentAuthor));
    if (documentOwner) infoRows.push(makeInfoRow('Owner', documentOwner));
    if (approvalDate) infoRows.push(makeInfoRow('Approval Date', approvalDate));
    if (effectiveDate) infoRows.push(makeInfoRow('Effective Date', effectiveDate));
    infoRows.push(makeInfoRow('Next Periodic Review Date', nextReviewDate));
    if (supersededDate) infoRows.push(makeInfoRow('Superseded Date', supersededDate));
